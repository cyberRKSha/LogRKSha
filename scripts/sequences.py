...
OUTPUT_FILE = os.path.join(settings.DATA_DIR, "training_sequences.pkl")
...
if __name__ == "__main__":
    ...
    if not df_logs.empty:
        ...
        embedder.save(settings.EMBEDDER_PATH)
        print(f"✅ Sentence embedder saved to {settings.EMBEDDER_PATH}")
        ...
        config.load(os.path.join(settings.PROJECT_ROOT, "drain3.ini"))
        ...
        with open(settings.DRAIN_MODEL_PATH, "wb") as f:
            dill.dump(miner, f)
        print(f"✅ Drain miner trained and saved to {settings.DRAIN_MODEL_PATH}")
